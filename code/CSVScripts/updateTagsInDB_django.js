...
function updateTagRatings() {
    var lineReader = require('readline').createInterface({
        input: require('fs').createReadStream('CSV/ratings_grouped.csv', 'utf8')
    });
    ...
    lineReader.on('line', function (line) {
        ...
        //var buff   = Buffer.from(JSON.stringify(variables[2]));
        var buff   = new Buffer.from(variables[1], 'utf8');

        var iid = variables[0];
       // var freq = variables[0];

        var tag = iconv.decode(iconv.decode(buff, 'ISO-8859-1'), 'UTF-8');
        var rating1 = variables[2];
        var rating2 = variables[3];
