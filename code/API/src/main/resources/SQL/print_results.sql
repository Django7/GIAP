...
WHERE
  length(tag) = 0 OR
  uid NOT IN
  (438, 44, 48, 63, 64, 71, 73, 74, 146, 203, 230, 244, 247, 250, 253, 272, 304, 313, 349, 380, 386, 389, 44, 47, 69, 90, 101, 103, 111, 120, 123, 157, 159, 166, 189, 195, 263, 273, 351, 395, 412, 429, 51, 61, 104, 115, 125, 151, 339, 407, 498, 55, 59, 105, 132, 135, 155, 216, 301, 341, 353, 398, 426, 441, 449, 456, 384, 461, 488, 502
  )
  OR uid < 44;
...
# get average quality (basic + extra)
SELECT
  uid,
  avg((rating + rating_2) / 2) AS avg_rating
FROM image_tags
...
# Demo questionnaire time
SELECT
  uid,
  duration,
  convert(substring(duration, 2, length(duration) - 2), UNSIGNED INT)
FROM questionnaires_duration
WHERE quest_name = '"demographics"';
